...
**Two anchors.** The forecast START = **`last_fact + 1`** (the day after the last fact) — so there is NEVER a
gap between facts and forecast, even when the requested date runs ahead of the available data. The Contract
Year is still cut day-precise on **`as_of`**, and the horizon = **`as_of + FORECAST_HORIZON_YEARS`**. When
`as_of` is later than the last fact, the forecast simply begins right after the facts and its contract years
are cut on the as_of day.

| # | Rule | How it is enforced |
|---|------|--------------------|
| 1 | Facts are the past, up to the last available day | Actuals from flight history; upper bound `first_seen < as_of`. FlightRadar is loaded up to **`today − 1`** (real yesterday; the current day is never queried; the window is NOT tied to as_of). The coverage ledger only advances to where flights actually landed (recent empty days stay re-fetchable). Flights newer than Cirium's latest revision are attributed via **carry-forward** (the latest reference snapshot), so a recently-flown tail is not lost. |
| 2 | Forecast is right after the facts → future | The forecast is anchored to **`last_fact + 1`**; the horizon is `as_of + FORECAST_HORIZON_YEARS`. |
| 3 | No day is both a fact and a forecast | Facts `≤ last_fact`, forecast `≥ last_fact + 1` → no overlap. |
| 4 | **No gap** between facts and forecast | The forecast starts exactly on `last_fact + 1` — no hole at any data freshness (the requested date cannot drag the start into the future). Plus a `k≥1` floor so proration rounding can't drop the boundary month. |
...
The Contract Year everywhere uses the `as_of` anchor (actuals re-stamp + forecast), so both halves of the
report bucket into the same contract years (the anchor day is the LAST day of its CY: `16-Sep → CY closes`,
`17-Sep → new CY`). `powerbi.z_dates_acys` derives the anchor day from **`max("Date")`** (the horizon =
`as_of.day`), NOT from the first forecast date — because the forecast now starts at `last_fact + 1`, which can
be earlier than `as_of`. Edge dates (29-Feb of a leap year) are clamped to the month length so `make_date`
never overflows.
